    ...
    ILogger logger) : IBuilder<IHmi>
{
    ...
    {
        var config = entry.Deserialize(InterfaceConfigurationContext.Default.GamepadConfiguration);
        ...
        var device = system.Claim(entry.ToString(), config.SerialNumber, config.Deadzone, config.Rumble);
        return Task.FromResult<IHmi>(new Shared.Gamepad(config, device, mixer, cameras.Get, logger, config.Pad));
    }
}
