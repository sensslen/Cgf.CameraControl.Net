...
**Help → Third-party licences** lists every package the application ships. The list is generated by
nuget-license, committed as `src/Cgf.CameraControl.App/Licenses/third-party-licenses.json`, and
compiled into the binary by `Cgf.CameraControl.Licenses.Generator`, so showing it costs nothing at
run time and it cannot drift from what is actually shipped. The same run fails the build on a licence
the project has not accepted. Regenerate it after changing a package reference:
